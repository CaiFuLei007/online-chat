...
### 阶段 4：群聊模块
- [x] 4.1 创建群、按群名模糊搜索（分页）
- [x] 4.2 加群申请、群主审批（同意/拒绝）
- [x] 4.3 退群、群成员列表
- [x] 4.4 注销群（群主权限，事务硬删群/成员/消息）
- [x] 4.5 我加入的群列表

### 阶段 5：消息模块
- [x] 5.1 单聊消息收发（落库 + 序号 + 在线投递/离线暂存 + ack）
- [x] 5.2 群聊消息收发（落库 + 广播在群在线成员 + 离线暂存）
- [x] 5.3 上线后拉取离线消息并投递
- [x] 5.4 历史消息分页查询接口（游标翻页）

### 阶段 6：管理模块
- [x] 6.1 超管查看全部群列表（元数据 + 分页）
- [x] 6.2 超管注销任意群（复用注销逻辑 + role 校验）

### 阶段 7：前端
- [x] 7.1 注册/登录页（验证码流程）
- [x] 7.2 主界面布局（好友/群列表、会话区、搜索）
- [x] 7.3 WebSocket 客户端封装（重连、心跳、收发）
- [x] 7.4 单聊/群聊界面 + 历史滚动加载
- [x] 7.5 申请通知与审批交互
- [x] 7.6 超管管理面板
